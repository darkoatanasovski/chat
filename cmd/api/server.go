...
func (a *App) registerDataRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("PATCH /channels/{id}", a.instrument("update_channel", a.requireAuth(a.handleUpdateChannel)))
	mux.HandleFunc("POST /channels/{id}/join", a.instrument("join_channel", a.requireAuth(a.handleJoinChannel)))
	mux.HandleFunc("GET /search/messages", a.instrument("search_app_messages", a.requireAuth(a.handleSearchAppMessages)))
	mux.HandleFunc("GET /search/channels", a.instrument("search_channels", a.requireAuth(a.handleSearchChannels)))
	...
	mux.HandleFunc("DELETE /channels/{id}/messages/{message_id}/link-preview", a.instrument("remove_link_preview", a.requireAuth(a.handleRemoveLinkPreview)))
