...
func TestConnectHandler_RelayTyping_DeliversToOtherMembers(t *testing.T) {
	...
	delivery := NewDelivery(hub, cache, nil, NewRegistry(client), NewPublisher(client), discardLogger())
	h := NewConnectHandler(nil, hub, NewRegistry(client), delivery, "eu-test", nil, discardLogger())
